...
const AgentEdit = () => {
  ...
  const allowNSFW = watch('allowNSFW');
  ...
  useEffect(() => {
    if (agentData) {
      ...
      reset({
        ...
        enableFollowupQuestions: agent.enableFollowupQuestions,
        isPublic: agent.isPublic,
        allowNSFW: agent.allowNSFW || false,
        nsfwCategories: agent.nsfwCategories || [],
        knowledgeBases: agent.knowledgeBases?.map(kb => kb._id || kb) || [],
      });
      
      setSelectedKnowledgeBases(agent.knowledgeBases?.map(kb => kb._id || kb) || []);
      setSelectedNSFWCategories(agent.nsfwCategories || []);
    }
  }, [agentData, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to update agent. Please try again.');
      setIsSubmitting(false);
    }
  };

  const isLoading = agentLoading || modelsLoading || knowledgeBasesLoading;
  const hasError = agentError || modelsError || knowledgeBasesError;
  ...
  return (
    <div className="container px-4 mx-auto">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-semibold text-gray-800 dark:text-white">Edit Agent</h1>
      </div>
